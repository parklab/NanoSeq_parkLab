...
rule check_efficiency:
    ...
    output:
        read_bundles="efficiency/{donor}.RBs",
        read_bundles_gc_inserts="efficiency/{donor}.RBs.GC_inserts.tsv",
        read_bundles_pdf="efficiency/{donor}.RBs.pdf",
        efficiency_stats="efficiency/{donor}.tsv",
    benchmark:
        "benchmarks/check_efficiency/{donor}.txt"
    log:
        "logs/check_efficiency/{donor}.log"
    ...
    shell:
        ...
        -out efficiency/{wildcards.donor} \
        -ref {params.fasta} 
        """
        # 2> {logs} 1> {logs}
...
rule coverage_histogram_controlBam:
    input:
        ...
        ctrl_bam=get_donor_expected_control_bam,
    output:
        coverage=expand("{donor}.runNanoSeq/tmpNanoSeq/cov/{chroms}.done",chroms=CHROMS,allow_missing=True),
    benchmark:
        "benchmarks/coverage_histogram_controlBam/{donor}.txt"
    log:
        "logs/coverage_histogram_controlBam/{donor}.log"
    params: # a preset for the ultrashear or covaris libraries
        fasta=config["fasta"],
        runNanoSeqDir = lambda wildcards: f"{wildcards.donor}.runNanoSeq",
    resources:
        mem_mb=10000,
    ...
    shell:
        ...
        # newDir="{wildcards.donor}.runNanoSeq/"
...
rule run_diagnostic_coverage:
    input:
        coverage=expand("{donor}.runNanoSeq/tmpNanoSeq/cov/{chroms}.done",chroms=CHROMS,allow_missing=True),
    output:
        coverage="{donor}.runNanoSeq/tmpNanoSeq/diagnostic/{donor}.tsv",
        coverage_table="{donor}.runNanoSeq/tmpNanoSeq/diagnostic/{donor}.bulk_coverage_diagnostic.logistic_pred.txt",
        coverage_model="{donor}.runNanoSeq/tmpNanoSeq/diagnostic/{donor}.bulk_coverage_diagnostic.logistic_fit.rds",
        threshold="{donor}.runNanoSeq/tmpNanoSeq/diagnostic/{donor}.bulk_coverage_diagnostic.threshold.txt",
    benchmark:
        "benchmarks/run_diagnostic_coverage/{donor}.txt"
    log:
        "logs/run_diagnostic_coverage/{donor}.log"
    params: # a preset for the ultrashear or covaris libraries
        fasta=config["fasta"],
    ...
    shell:
        ...
        nanoseqDir={wildcards.donor}.runNanoSeq/tmpNanoSeq
        ...
        outputName=$nanoseqDir/diagnostic/{wildcards.donor}.bulk_coverage_diagnostic
...
rule partition_coverage:
    input:
        duplex_bam=get_donor_expected_sample_bam,
        ctrl_bam=get_donor_expected_control_bam,
        histCoverage=expand("{donor}.runNanoSeq/tmpNanoSeq/cov/{chroms}.done",chroms=CHROMS,allow_missing=True),
    output:
        coverage="{donor}.runNanoSeq/tmpNanoSeq/part/args.json",
    benchmark:
        "benchmarks/partition_coverage/{donor}.txt"
    log:
        "logs/partition_coverage/{donor}.log"
    params: # a preset for the ultrashear or covaris libraries
        fasta=config["fasta"],
        n_partitions=60,
        jobs=n_jobs_partitioned,
        indir=lambda wildcards: f"{wildcards.donor}.runNanoSeq",
    resources:
        mem_mb=10000,
        # runtime=240,
...
rule start_dsa:
    ...
    output:
        jobDone=expand("{donor}.runNanoSeq/tmpNanoSeq/dsa/{job}.start",job=jobs_partitioned,allow_missing=True), # job=n_jobs_partitioned,allow_missing=True),
        jobToIntvl="{donor}.runNanoSeq/tmpNanoSeq/dsa/job_to_intvl.txt",
    params:
    ...
    shell:
        ...
        for i in {params.jobs}; do
            ...
            echo -e "$intvl" > {wildcards.donor}.runNanoSeq/tmpNanoSeq/dsa/$i.start
            echo -e "$i\t$intvl" >> {output.jobToIntvl}
        done
        """
        #grep -c "^" {input.intvl_list} > {wildcards.donor}.runNanoSeq/tmpNanoSeq/dsa/nfiles
...
rule add_dsa_args:
    ...
    input:
        intvl_list=rules.list_intervals.output.intvl_list,
    output:
        nfiles="{donor}.runNanoSeq/tmpNanoSeq/dsa/nfiles",
        argsJson="{donor}.runNanoSeq/tmpNanoSeq/dsa/args.json",
    params:
        jobs=jobs_partitioned
    benchmark:
        "benchmarks/add_dsa_args/{donor}.txt"
    log:
        "logs/add_dsa_args/{donor}.log"
    resources:
        mem_mb=5000,
        # runtime=10,
...
# MODIFY TO HANDLE DONOR-LEVEL TABLE
rule dsa_bed_per_partition:
    input:
        # sampleConfig                            =   rules.associate_sample_control.output[0],
        sampleConfig                            =   rules.list_donor_info.output[0],   
        duplex_bam                              =   get_donor_expected_sample_bam,
        ctrl_bam                                =   get_donor_expected_control_bam,
        coverage                                =   rules.partition_coverage.output.coverage, # "{donor}.runNanoSeq/tmpNanoSeq/part/args.json",
        job                                     =   "{donor}.runNanoSeq/tmpNanoSeq/dsa/{job}.start", # something wrong with this job not existing...
    ...
    output:
       coverage                                 =   "{donor}.runNanoSeq/tmpNanoSeq/dsa/{job}.dsa.bed.gz", # job=n_jobs_partitioned,allow_missing=True),
       jobDone                                  =   "{donor}.runNanoSeq/tmpNanoSeq/dsa/{job}.done", # job=n_jobs_partitioned,allow_missing=True),
    benchmark:
        "benchmarks/dsa_bed_per_partition/{donor}.{job}.txt"
    log:
        "logs/dsa_bed_per_partition/{donor}.{job}.log"
    params: # a preset for the ultrashear or covaris libraries
        fasta                                   =   config["fasta"],
        indir                                   =   lambda wildcards: f"{wildcards.donor}.runNanoSeq",
        # snp                                     =   SNP,
...
rule start_varCall:
    ...
    input:
        ...
        argsJson                                =   rules.add_dsa_args.output.argsJson,
    output:
        nfiles="{donor}.runNanoSeq/tmpNanoSeq/var/nfiles",
        argsJson="{donor}.runNanoSeq/tmpNanoSeq/var/args.json",
    params:
        jobs=jobs_partitioned
    benchmark:
        "benchmarks/start_varCall/{donor}.txt"
    log:
        "logs/start_varCall/{donor}.log"
    resources:
        mem_mb=5000,
        # runtime=10,
        runtime=2,
...
varPath="{donor}.runNanoSeq/tmpNanoSeq/var/{job}"
rule varCall_per_partition:
    input:
        ...
        coverage                            =   rules.partition_coverage.output.coverage, # "{donor}.runNanoSeq/tmpNanoSeq/part/args.json",
        job                                 =   "{donor}.runNanoSeq/tmpNanoSeq/dsa/{job}.start",
    ...
    benchmark:
        "benchmarks/varCall_per_partition/{donor}.{job}.txt"
    log:
        "logs/varCall_per_partition/{donor}.{job}.log"
    params: # a preset for the ultrashear or covaris libraries
...
# Indel calling
indelPath="{donor}.runNanoSeq/tmpNanoSeq/indel"
rule start_indelCall:
    ...
    params:
        jobs                    =   jobs_partitioned
    benchmark:
        "benchmarks/start_indelCall/{donor}.txt"
    log:
        "logs/start_indelCall/{donor}.log"
    resources:
        mem_mb                  =   5000,
        # runtime                 =   10,
        runtime                 =   10,
...
rule indel_propose_per_partition:
    ...
    benchmark:
        "benchmarks/indel_propose_per_partition/{donor}.{job}.txt"
    log:
        "logs/indel_propose_per_partition/{donor}.{job}.log"
    params: # a preset for the ultrashear or covaris libraries
...
rule indel_call_per_partition:
    ...
    benchmark:
        "benchmarks/indel_call_per_partition/{donor}.{job}.txt"
    log:
        "logs/indel_call_per_partition/{donor}.{job}.log"
    params:
        fasta                   =   config["fasta"],
        indir                   =   lambda wildcards: f"{wildcards.donor}.runNanoSeq",
    resources:
...
rule indel_verify_per_partition:
    ...
    benchmark:
        "benchmarks/indel_verify_per_partition/{donor}.{job}.txt"
    log:
        "logs/indel_verify_per_partition/{donor}.{job}.log"
    params:
        fasta                   =   config["fasta"],
