...
rule extract_tags:
    input:
        expand(f"{FASTQ_DIR}/{{sample}}_{{pe}}.fq.gz", pe=PAIRED_ENDS, allow_missing=True)
    output:
        expand("extracted_tags_duplex_fq/{sample}_{pe}.fq.gz",pe=PAIRED_ENDS,allow_missing=True)
    benchmark:
...
rule prepare_RcMcOd_tags:
    ...
    shell:
        ...
        ulimit -c unlimited
        ulimit -n 8192
        PATH=$PATH:/n/data1/hms/dbmi/park/yujie/biobambam2/src
...
rule mark_read_bundles:
    ...
    shell:
        """
        PATH=$PATH:$PWD/bin/
        bamaddreadbundles -I {input} -O {output.outbam} || exit 1
