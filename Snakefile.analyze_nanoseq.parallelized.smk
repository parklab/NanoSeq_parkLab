# rule mark_read_bundles added after rule mark_read_bundles!
...
def check_ctrl_bam_exists(wildcards):
    file_path = f"control_bam/{wildcards.sample}.RG.ctrl.bam"
...
if config["dilute_ctrl_bam"]:
    rule dilute_normal:
        input:
            RGctrl_bam=rules.control_bam_add_readbundle.output.RG_control,
        output:
        ...
        shell: 
            """
            PATH=$PATH:$PWD/bin/
            randomreadinbundle -I {input.RGctrl_bam} -O {output.diluted_control} && \
...
rule dsa_bed_per_partition:
    input:
        ...
        job                                     =   lambda wc: f"{wc.sample}.runNanoSeq/tmpNanoSeq/dsa/{wc.job}.start", # something wrong with this job not existing...
    ...
    shell:
        ...
        # format the interval
        chrom=$(echo -e $intvl | rev | cut -d'_' -f3- | rev)
        startPos=$(echo -e $intvl | rev | cut -d'_' -f2 | rev)
        endPos=$(echo -e $intvl | rev | cut -d'_' -f1 | rev) 
        # navigate to output directory
        cd {input.indir}
        
       # echo "DEBUG: chrom=$chrom startPos=$startPos endPos=$endPos jobIndex=$jobIndex" >&2        
        echo -e "running dsa..."
        dsa \
        -A ../{input.ctrl_bam} \
        -B ../{input.duplex_bam} \
