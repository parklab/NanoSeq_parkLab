# This workflow will isolate mitochondrial reads from a duplex sequencing dataset and perform variant calling on the mitochondrial genome. It is designed to be run on a single sample, but can be modified to run on multiple samples.
...
rule all:
    input:
        ...
        # per-base coverage tracks + Circos plots (keyed on TestBamID, since
        # donors sharing a test BAM have identical mitochondrial coverage)
        ...
        # Mitochondrial variant calling (first pass, unmasked). Lambda-wrapped:
        # this rule is defined before the includes, so the donor list the
        # calling workflow computes is not in scope until DAG-build time.
        lambda wildcards: mito_call_targets()
...
# AUG 12 2026: JUST GET COVERAGE STATISTICS OVER THE MITOCHONDRIAL GENOME, ISOLATING A4S2 DUPLEXES
